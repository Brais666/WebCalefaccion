...
<div class="content">
   ...
   <div class="row">
        <div class="col-lg-11">
                <h2>Rutas de Catalunya y Madrid</h2>
        </div>
    ...
    <table class="table table-bordered table-striped table-hover clientes">
        ...
        @php
            $i = 0;
        @endphp
        @foreach ($rutacatmads as $rutacatmad)
            <tr>
                <td>{{ ++$i }}</td>
                <td>{{ $rutacatmad->idzona }}</td>
                <td>{{ $rutacatmad->poblacion }}</td>
                <td>{{ $rutacatmad->codigopost }}</td>
                <td>{{ $rutacatmad->nombreruta }}</td>
                <td>{{ $rutacatmad->dia }}</td>
                <td>
                    <form action="{{ route('rutas.destroy',$rutacatmad->id) }}" method="POST">
                        <a class="btn btn-info" href="{{ route('rutas.show',$rutacatmad->id) }}"><i class="fas fa-eye"></i></a>
                        <a class="btn btn-primary" href="{{ route('rutas.edit',$rutacatmad->id) }}"><i class="fas fa-edit"></i></a>
                        @csrf
                        @method('DELETE')
